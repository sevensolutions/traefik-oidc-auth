...
func (toa *TraefikOidcAuth) isAuthorized(claims map[string]interface{}) bool {
	...
	if authorization.AssertClaims != nil && len(authorization.AssertClaims) > 0 {
		...
		if err != nil {
			toa.logger.Log(logging.LevelWarn, "Error whilst marshalling claims object: %s", err.Error())
			return false
		}
	...
	assertions:
		for _, assertion := range authorization.AssertClaims {
			...
			if err != nil {
				toa.logger.Log(logging.LevelWarn, "Error whilst parsing path for claim %s in token claims: %s", assertion.Name, err.Error())
				return false
			} else if len(value) == 0 {
				toa.logger.Log(logging.LevelWarn, "Unauthorized. Unable to find claim %s in token claims.", assertion.Name)
				toa.logAvailableClaims(claims)
				return false
			}

			if len(assertion.AllOf) == 0 && len(assertion.AnyOf) == 0 {
				toa.logger.Log(logging.LevelDebug, "Authorized claim %s. No assertions were defined and claim exists", assertion.Name)
				continue assertions
			}
		...
		matches:
			for _, val := range value {
				...
				if err != nil {
					toa.logger.Log(logging.LevelError, "Error whilst unpacking json node: %s", err.Error())
					continue matches
				}
				...
				case []interface{}:
					...
					if len(assertion.AnyOf) > 0 {
						for _, assert := range assertion.AnyOf {
							if slices.Contains(mapped, assert) {
								toa.logger.Log(logging.LevelDebug, "Authorized claim %s: Found value %s which is any of [%s]", assertion.Name, assert, strings.Join(assertion.AnyOf, ", "))
								continue assertions
							}
						}
						continue matches
					}
					toa.logger.Log(logging.LevelDebug, "Authorized claim %s: Found all values of [%s]", assertion.Name, strings.Join(assertion.AllOf, ", "))
					continue assertions
			...
			if len(assertion.AnyOf) > 0 && anyMatch && len(assertion.AllOf) > 0 && !slices.Contains(allMatches, false) {
				toa.logger.Log(logging.LevelDebug, "Authorized claim %s: Found any value of [%s] and all values of [%s]", assertion.Name, strings.Join(assertion.AnyOf, ", "), strings.Join(assertion.AllOf, ", "))
				continue assertions
			} else if len(assertion.AnyOf) > 0 && anyMatch && len(assertion.AllOf) == 0 {
				toa.logger.Log(logging.LevelDebug, "Authorized claim %s: Found any value of [%s]", assertion.Name, strings.Join(assertion.AnyOf, ", "))
				continue assertions
			...
			if len(assertion.AllOf) > 0 && len(assertion.AnyOf) > 0 {
				toa.logger.Log(logging.LevelWarn, "Unauthorized. Expected claim %s to contain any value of [%s] and all values of [%s]", assertion.Name, strings.Join(assertion.AnyOf, ", "), strings.Join(assertion.AllOf, ", "))
			} else if len(assertion.AllOf) > 0 {
				toa.logger.Log(logging.LevelWarn, "Unauthorized. Expected claim %s to contain all values of [%s]", assertion.Name, strings.Join(assertion.AllOf, ", "))
			} else if len(assertion.AnyOf) > 0 {
				toa.logger.Log(logging.LevelWarn, "Unauthorized. Expected claim %s to contain any value of [%s]", assertion.Name, strings.Join(assertion.AnyOf, ", "))
			}
...
func (toa *TraefikOidcAuth) logAvailableClaims(claims map[string]interface{}) {
	toa.logger.Log(logging.LevelDebug, "Available claims are:")

	for key, val := range claims {
		toa.logger.Log(logging.LevelDebug, "  %v = %v", key, val)
	}
}
